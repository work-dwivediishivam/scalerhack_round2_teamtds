...
- README links to the Space, demo, training evidence, artifacts, and writeup.
- No large video files committed into the HF Space repository.
- Experiment tracking enabled by default through `--report-to tensorboard` in the GRPO training script.

## Form Notes

- Only the team leader should submit.
- Use the registered team leader email.
- Only the first submission counts.
- Commits after the deadline are not considered.
