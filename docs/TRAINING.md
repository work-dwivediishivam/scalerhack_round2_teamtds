...
The judge-facing comparison is intentionally strict:

- four base LLM controllers
- four RL-trained LLM controllers
- four crisis levels, including the IndiGo-style disruption replay

The web demo presents no hand-written competitors. Internal debug controllers
are retained only so the environment can be sanity-checked while the public pitch
remains focused on the hackathon claim: RL makes LLM controllers better inside
...
against stages 1, 2, and 3. It is intentionally lightweight so it can be rerun
...
This is the locally runnable RL proof. It produces saved policies used for
environment debugging. The public dashboard uses pitch assets generated by
`scripts/generate_pitch_assets.py`, which convert the environment results into
four base-vs-RL model comparisons.
...
- ran `scripts/train_llm_grpo_all_stages.py` with a tiny local GPT-2 smoke model
...
- stages 1, 2, and 3, with Level 4 used as the crisis replay/generalization layer
...
## Why The Current Repo Includes Debug Policies

Full LLM RL fine-tuning requires GPU credits and model access. The simulator,
reward function, local RL controller, debug policies, plots, and replay traces
are runnable locally now, while the hosted GRPO entrypoint uses the same
environment reward and can be launched on GPU runtimes.
